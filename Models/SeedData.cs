...
namespace MVCBook.Models
{
    public class SeedData
    {
        public static async Task CreateUserRoles(IServiceProvider serviceProvider)
        {
            ...
            var UserManager = serviceProvider.GetRequiredService<UserManager<MVCBookUser>>();
            IdentityResult roleResult;
            //Add Admin Role
            var roleCheck = await RoleManager.RoleExistsAsync("Admin");
            if (!roleCheck) { roleResult = await RoleManager.CreateAsync(new IdentityRole("Admin")); }
            MVCBookUser user = await UserManager.FindByEmailAsync("admin@mvcbook.com");
            if (user == null)
            {
                var User = new MVCBookUser();
            ...
            MVCBookUser user1 = await UserManager.FindByEmailAsync("user1@mvcbook.com");
            if (user1 == null) 
            {
                var User = new MVCBookUser();
